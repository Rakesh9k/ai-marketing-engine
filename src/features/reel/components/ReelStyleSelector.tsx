...
import { REEL_STYLE_OPTIONS } from '@/features/reel/constants';
import type { ReelStyle } from '@/types';

interface ReelStyleSelectorProps {
  value: ReelStyle | null;
  onChange: (style: ReelStyle) => void;
}

export function ReelStyleSelector({ value, onChange }: ReelStyleSelectorProps) {
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
      {REEL_STYLE_OPTIONS.map((option) => {
