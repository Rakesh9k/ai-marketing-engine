...
export const onUserCreatedHandler: CloudFunction<UserRecord> = user().onCreate(
  async (userRecord) => {
    ...
    try {
      ...
      try {
        ...
        // Track signup event - new user account successfully created
        await trackEvent(ANALYTICS_EVENTS.SIGNUP, userId);
      } catch (claimsError) {
...
export const onUserDeletedHandler: CloudFunction<UserRecord> = user().onDelete(
  async (userRecord) => {
